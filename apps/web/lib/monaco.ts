...
// Configure Monaco loader for dynamic import with production fallbacks
export const configureMonacoLoader = () => {
  if (isLoaderConfigured) return;
  ...
  try {
    ...
    // Configure loader to handle errors
    loader.init().catch((err) => {
      console.warn("Monaco loader initialization warning (non-fatal):", err);
    });
...
export const initializeMonaco = (monaco: Monaco) => {
  if (isMonacoInitialized || !monaco) return;
