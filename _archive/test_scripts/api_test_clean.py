...
def test_code_execution():
    ...
    try:
        ...
        result = log_test(
            category="Code Execution",
            endpoint="/api/v1/execute/run (syntax error)",
            method="POST",
            status=response.status_code,
            response_time=elapsed,
            payload=payload,
    ...
    try:
        ...
        result = log_test(
            category="Code Execution",
            endpoint="/api/v1/execute/run (timeout)",
            method="POST",
            status=response.status_code,
            response_time=elapsed,
            payload=payload,
...
def test_verification_endpoints():
    ...
    test_slug = "week1_day1_calculate_sum"  # Default fallback
    try:
        ...
        if problems and len(problems) > 0:
            test_slug = problems[0]["slug"]
        print(f"Using test slug: {test_slug}")
    except Exception as e:
        print(f"Using default test slug: {test_slug} (error: {e})")
...
def test_curriculum_endpoints():
    ...
    try:
        ...
        if has_data and len(resp_data) > 0:
            first_week_slug = resp_data[0].get("slug")
    except Exception as e:
        elapsed = time.time() - start
        result = log_test(
            category="Curriculum",
    ...
    try:
        ...
        if has_data and len(resp_data) > 0:
            first_problem_slug = resp_data[0].get("slug")
    except Exception as e:
        elapsed = time.time() - start
        result = log_test(
            category="Curriculum",
...
def test_rate_limits():
    ...
    print("=" * 60)
    print("RATE LIMIT TESTING (Limited to 5 requests)")
    print("=" * 60)
    
    print("\n--- Testing /api/v1/execute/run rate limit ---")
    ...
    for i in range(5):
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                print(f"   Request {i+1}: {response.status_code} ({elapsed*1000:.0f}ms)")
        except Exception as e:
            elapsed = time.time() - start
            responses.append({"status": 0, "time": elapsed, "error": str(e)})
            print(f"   Request {i+1}: Error - {e}")
    
    avg_time = sum(r["time"] for r in responses) / len(responses) if responses else 0
    result = log_test(
...
def generate_report():
    ...
    for cat, stats in sorted(categories.items()):
        ...
        icon = "[OK]" if cat_pass_rate == 100 else "[WARN]" if cat_pass_rate >= 50 else "[FAIL]"
...
def main():
    ...
    if stats["pass_rate"] >= 90:
        ...
        print("OVERALL STATUS: GOOD WITH ISSUES")
    else:
